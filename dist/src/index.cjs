...
var AIAnalyzer = class {
  ...
  async analyzeFile(filePath) {
    ...
    const extension = (0, import_path.extname)(filePath).toLowerCase();
...
var AIFixer = class {
  ...
  async applySuggestion(suggestion, options) {
    ...
    const currentContent = await (0, import_promises2.readFile)(filePath, "utf-8");
    ...
    await (0, import_promises2.writeFile)(filePath, newContent);
  }
  /**
  ...
  async getCodeContext(issue, projectPath) {
    try {
      const fullContent = await (0, import_promises2.readFile)(issue.file, "utf-8");
...
var FileProcessor = class {
  ...
  async walkDirectory(dir, pattern, ignorePatterns, files, currentDepth = 0) {
    ...
    try {
      ...
      for (const entry of entries) {
        ...
        const relativePath = (0, import_path2.relative)(process.cwd(), fullPath);
  ...
  async findFilesByExtension(rootPath, extension, ignorePatterns) {
    const files = [];
    try {
      const entries = await (0, import_promises3.readdir)(rootPath, { withFileTypes: true });
      for (const entry of entries) {
        const fullPath = (0, import_path2.join)(rootPath, entry.name);
        const relativePath = (0, import_path2.relative)(process.cwd(), fullPath);
        if (this.shouldIgnorePath(relativePath, ignorePatterns)) {
          continue;
        }
        if (entry.isDirectory()) {
          ...
          files.push(...subFiles);
        } else if (entry.isFile() && (0, import_path2.extname)(fullPath) === extension) {
          files.push(fullPath);
        }
      }
    } catch (error) {
  ...
  async getFileInfo(filePath) {
    ...
    return {
      path: filePath,
      relativePath: (0, import_path2.relative)(process.cwd(), filePath),
      extension,
      size: stats.size,
      lines,
      hasTests,
  ...
  async createBackup(filePaths, backupDir) {
    ...
    for (const filePath of filePaths) {
      try {
        ...
        const backupPath = (0, import_path2.join)(backupDir, relativePath);
...
var AIRefactor = class {
  ...
  async generateReport(result, outputPath) {
    ...
    if (outputPath) {
      await (0, import_promises4.writeFile)(outputPath, report);
